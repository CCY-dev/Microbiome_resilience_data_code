...
merge_dist <- merge_dist %>% 
  mutate(Treatment = case_when(
    Study %in% c("Voigt", "Alien_before_antibiotics", "HMP_HHS", "Suez_before_antibiotics") ~ "Non-treated",
...
pdf("Suez_Palleja_Voigt_HMPHHS_shift_drift_all_timepoints_species.pdf", width = 8, height = 6)
# Raw data
ggplot(merge_dist, aes(Day^(1/2), Distance)) +
  geom_point(aes(color = Study), alpha = 0.8) +
  scale_color_manual(values = c("Voigt" = "gray54" ,
  ...
  geom_line(aes(group = Individual, color = Study), alpha = 0.5, size = 0.6) +
  theme_light() +
...
ggplot(merge_dist, aes(Day^(1/2), Distance)) +
  ...
  geom_smooth(aes(group = Study, color = Study), method = "loess", se = F, lty = "D3") +
  theme(legend.position = "bottom", legend.direction = "horizontal",
        axis.text=element_text(size=10),
        axis.title=element_text(size=14)) +
  guides(color = guide_legend(title="Group")) +
...
ggplot(merge_dist, aes(Day^(1/2), Distance)) +
  ...
  scale_color_manual(values = c("Voigt" = "gray54" ,
                                ...
                                "Alien_after_antibiotics"  = "coral",
                                "Palleja"= "cornflowerblue")) +
  scale_fill_manual(values = c("Voigt" = "gray54" ,
                               "Alien_before_antibiotics" = "black",
                               "HMP_HHS" = "darkgoldenrod1",
                               "Suez_after_antibiotics" = "yellowgreen",
                               "Suez_before_antibiotics" = "mediumturquoise",
  ...
  theme(legend.position = "bottom", legend.direction = "horizontal",
        axis.text=element_text(size=10),
        axis.title=element_text(size=14)) +
  guides(color = guide_legend(title="Group"), fill = "none") +
  labs(x = "Day (square-rooted)")
...
ggplot(merge_dist, aes(Day^(1/2), Distance)) +
  ...
  theme(legend.position = "bottom", legend.direction = "horizontal",
        axis.text=element_text(size=10),
        axis.title=element_text(size=14)) +  guides(color = guide_legend(title="Treatment"), fill = "none") +
  labs(x = "Day (square-rooted)")
...
ggplot(merge_dist, aes(Day^(1/2), Distance)) +
  ...
  theme(legend.position = "bottom", legend.direction = "horizontal",
        axis.text=element_text(size=10),
        axis.title=element_text(size=14)) +  guides(color = guide_legend(title="Group")) +
  labs(x = "Day (square-rooted)")
dev.off()
