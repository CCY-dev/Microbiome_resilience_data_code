...
########### Load distance info ###########
dist_df_long_same <-read.table(file = "Palleja_Suez_GMM_bray_vegdist.txt",
                         header = T, sep = "\t", stringsAsFactors = F)
...
(cor_2 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Bacteroides,
                   method = "spearman"))
(g2 <- ggplot(ML_table_final, aes(x = Bacteroides, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
...
(g3 <- ggplot(ML_table_final, aes(x = Bifidobacterium, y = Dist_V1V2)) + 
    ...
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=3, y=0.75, size = 4,color = "red",
...
(g4 <- ggplot(ML_table_final, aes(x = Anaerostipes, y = Dist_V1V2)) + 
    ...
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=6, y=0.75, size = 4,color = "red",
...
(g5 <- ggplot(ML_table_final, aes(x = Lactobacillus, y = Dist_V1V2)) + 
    ...
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=1.1, y=0.75, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_5$estimate, 3), " , p=", round(cor_5$p.value, 4))))

(cor_6 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Romboutsia,
                   method = "spearman"))
(g6 <- ggplot(ML_table_final, aes(x = Romboutsia, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Romboutsia at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=1, y=0.75, size = 4,color = "black",
...
(cor_8 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Faecalibacterium,
                   method = "spearman"))
(g8 <- ggplot(ML_table_final, aes(x = Faecalibacterium, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
...
(cor_9 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Dehalococcoidales.gen..incertae.sedis,
                   method = "spearman"))
(g9 <- ggplot(ML_table_final, aes(x = Dehalococcoidales.gen..incertae.sedis, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
...
(cor_10 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Succinivibrio,
                    method = "spearman"))
(g10 <- ggplot(ML_table_final, aes(x = Succinivibrio, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Succinivibrio at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=7, y=0.75, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_10$estimate, 3), " , p=", round(cor_10$p.value, 4))))

(g1|g2)/(g3|g4)/(g5|g6) + plot_layout(guides = "collect")
#ggsave("Palleja_Suez_genus_predict_GMM_vegdist_correlation1.pdf", device = "pdf", width = 12, height = 7)

(g7|g8)/(g9|g10) + plot_layout(guides = "collect")
#ggsave("Palleja_Suez_genus_predict_GMM_vegdist_correlation2.pdf", device = "pdf", width = 12, height = 5)
...
cor_1 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Shannon,
                   method = "spearman")
cor_2 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Anaerostipes,
                  method = "spearman")
cor_3 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Bifidobacterium,
                  method = "spearman")
cor_4 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Lactobacillus,
                  method = "spearman")
cor_5 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Bacteroides,
                  method = "spearman")
cor_6 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Succinivibrio,
                  method = "spearman")
cor_7 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Romboutsia,
                  method = "spearman")
cor_8 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Faecalibacterium,
                  method = "spearman")
cor_9 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Dehalococcoidales.gen..incertae.sedis,
                  method = "spearman")
cor_10 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Fusicatenibacter,
                  method = "spearman")
