...
########### Load distance info ###########
dist_df_long_same <-read.table(file = "Palleja_Suez_kegg_module_rarefy495663_bray_vegdist.txt",
                         header = T, sep = "\t", stringsAsFactors = F)
...
#write.table(ML_table_final, "Palleja_Suez_genus_baseline_predict_KEGGmodule_vegdist_bray_forML.txt", sep = "\t",
#            row.names = F, col.names = T, quote = F)


###### Plotting
(cor_1 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Shannon,
                   method = "spearman"))
(g1 <- ggplot(ML_table_final, aes(x = Shannon, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Shannon at baseline v.s. distance") +
    theme_light() +
    labs(x = "Shannon") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=2.2, y=0.75, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_1$estimate, 3), " , p=", round(cor_1$p.value, 4))))

(cor_2 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Alistipes,
                   method = "spearman"))
(g2 <- ggplot(ML_table_final, aes(x = Alistipes, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Alistipes at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=4, y=0.75, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_2$estimate, 3), " , p=", round(cor_2$p.value, 4))))

(cor_3 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Bifidobacterium,
                   method = "spearman"))
(g3 <- ggplot(ML_table_final, aes(x = Bifidobacterium, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Bifidobacterium at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=3.2, y=0.75, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_3$estimate, 3), " , p=", round(cor_3$p.value, 4))))

(cor_4 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Anaerostipes,
                   method = "spearman"))
(g4 <- ggplot(ML_table_final, aes(x = Anaerostipes, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Anaerostipes at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=6, y=0.75, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_4$estimate, 3), " , p=", round(cor_4$p.value, 4))))

(cor_5 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Lactobacillus,
                   method = "spearman"))
(g5 <- ggplot(ML_table_final, aes(x = Lactobacillus, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Lactobacillus at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=1.2, y=0.75, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_5$estimate, 3), " , p=", round(cor_5$p.value, 4))))

(cor_6 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Bacteroides,
                   method = "spearman"))
(g6 <- ggplot(ML_table_final, aes(x = Bacteroides, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Bacteroides at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=10, y=0.75, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_6$estimate, 3), " , p=", round(cor_6$p.value, 4))))

(cor_7 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Firmicutes.gen..incertae.sedis,
                   method = "spearman"))
(g7 <- ggplot(ML_table_final, aes(x = Firmicutes.gen..incertae.sedis, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Firmicutes gen. inc.sedis at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=10, y=0.75, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_7$estimate, 3), " , p=", round(cor_7$p.value, 4))))

(cor_8 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Dehalococcoidales.gen..incertae.sedis,
                   method = "spearman"))
(g8 <- ggplot(ML_table_final, aes(x = Dehalococcoidales.gen..incertae.sedis, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Dehalococcoidales gen. inc. sedis at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=3.5, y=0.75, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_8$estimate, 3), " , p=", round(cor_8$p.value, 4))))

(cor_9 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Clostridiales.gen..incertae.sedis,
                   method = "spearman"))
(g9 <- ggplot(ML_table_final, aes(x = Clostridiales.gen..incertae.sedis, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Clostridiales gen inc. sedis at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=18, y=0.75, size = 4,color = "black",
...
(g10 <- ggplot(ML_table_final, aes(x = Faecalibacterium, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("Faecalibacterium at baseline v.s. distance") +
    theme_light() +
    labs(x = "Abundance") + 
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
...
#ggsave("Palleja_Suez_genus_predict_KEGGmodule_vegdist_correlation1.pdf", device = "pdf", width = 12, height = 7)

(g7|g8)/(g9|g10) + plot_layout(guides = "collect")
#ggsave("Palleja_Suez_genus_predict_KEGGmodule_vegdist_correlation2.pdf", device = "pdf", width = 12, height = 5)

#### Do FDR correction
cor_results <- as.data.frame(matrix(nrow = 10, ncol = 3))
colnames(cor_results) <- c("Feature", "Spearman_rho", "Spearman_p")
cor_results$Feature <- c("Shannon",
                         "Alistipes",
                         "Bifidobacterium",
                         "Anaerostipes",
                         "Lactobacillus",
                         "Bacteroides",
                         "Firmicutes.gen..incertae.sedis",
                         "Dehalococcoidales.gen..incertae.sedis",
                         "Clostridiales.gen..incertae.sedis",
                         "Faecalibacterium")
...
#### Do FDR correction for LOOCV result
cor_results <- as.data.frame(matrix(nrow = 10, ncol = 3))
colnames(cor_results) <- c("Feature", "Spearman_rho", "Spearman_p")
cor_1 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Shannon,
                  method = "spearman")
cor_2 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Bifidobacterium,
                  method = "spearman")
cor_3 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Alistipes,
                  method = "spearman")
cor_4 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Anaerostipes,
                  method = "spearman")
cor_5 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Bacteroides,
                  method = "spearman")
cor_6 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Lactobacillus,
                  method = "spearman")
cor_7 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Succinivibrio,
                  method = "spearman")
cor_8 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Firmicutes.gen..incertae.sedis,
                  method = "spearman")
cor_9 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Coprococcus,
                  method = "spearman")
cor_10 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$Dorea,
                   method = "spearman")
