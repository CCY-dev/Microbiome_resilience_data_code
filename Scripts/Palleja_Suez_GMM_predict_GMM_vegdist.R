...
########### Read Bray-curtis distance ###########
# Here I'm using bray-curtis cuz it's count data
distance <-read.table(file = "/Users/Jessica/Documents/Lab/Multiple_longitudinal/Microbe_resilience/Palleja_Suez_GMM_bray_vegdist.txt",
                         header = T, sep = "\t", stringsAsFactors = F)
...
#write.table(ML_table_final, "Palleja_Suez_GMM_baseline_predict_GMM_vegdist_bray_forML.txt", sep = "\t",
#            row.names = F, col.names = T, quote = F)

### Correlation between the features and Dist_V1V2
# This is for random forest result

(cor_1 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0042,
                   method = "spearman"))
(g1 <- ggplot(ML_table_final, aes(x = MF0042, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0042 baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=10, y=0.5, size = 4,color = "red",
...
(g2 <- ggplot(ML_table_final, aes(x = MF0048, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0048 baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=600, y=0.5, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_2$estimate, 3), " , p=", round(cor_2$p.value, 4))))

(cor_3 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0030,
                   method = "spearman"))
(g3 <- ggplot(ML_table_final, aes(x = MF0030, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0030 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=280, y=0.5, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_3$estimate, 3), " , p=", round(cor_3$p.value, 4))))

(cor_4 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0033,
                   method = "spearman"))
(g4 <- ggplot(ML_table_final, aes(x = MF0033, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0033 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=90, y=0.5, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_4$estimate, 3), " , p=", round(cor_4$p.value, 4))))

(cor_5 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0019,
                   method = "spearman"))
(g5 <- ggplot(ML_table_final, aes(x = MF0019, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0019 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=3, y=1, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_5$estimate, 3), " , p=", round(cor_5$p.value, 4))))

(cor_6 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0115,
                   method = "spearman"))
(g6 <- ggplot(ML_table_final, aes(x = MF0115, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0115 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=7, y=0.5, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_6$estimate, 3), " , p=", round(cor_6$p.value, 4))))

(cor_7 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0084,
                   method = "spearman"))
(g7 <- ggplot(ML_table_final, aes(x = MF0084, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0084 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=20, y=0.5, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_7$estimate, 3), " , p=", round(cor_7$p.value, 4))))

(cor_8 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0085,
                   method = "spearman"))
(g8 <- ggplot(ML_table_final, aes(x = MF0085, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0085 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=400, y=0.5, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_8$estimate, 3), " , p=", round(cor_8$p.value, 4))))

(cor_9 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0011,
                   method = "spearman"))
(g9 <- ggplot(ML_table_final, aes(x = MF0011, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0011 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=115, y=0.5, size = 4,color = "black",
             label=paste0("Spearman's rho=", round(cor_9$estimate, 3), " , p=", round(cor_9$p.value, 4))))

(cor_10 <- cor.test(ML_table_final$Dist_V1V2, ML_table_final$MF0132,
                    method = "spearman"))
(g10 <- ggplot(ML_table_final, aes(x = MF0132, y = Dist_V1V2)) + 
    geom_point(alpha = 0.6, color = "cornflowerblue", size = 3, aes(shape = Study)) +
    ggtitle("MF0132 at baseline v.s. V1_V2 distance") +
    theme_light() +
    theme(panel.grid.minor = element_blank()) +
    geom_smooth(method = "lm", se = F, color = "cornflowerblue", alpha = 0.8) +
    annotate(geom="text", x=60, y=0.5, size = 4,color = "red",
             label=paste0("Spearman's rho=", round(cor_10$estimate, 3), " , p=", round(cor_10$p.value, 4))))

(g1|g2)/(g3|g4)/(g5|g6) + plot_layout(guides = "collect")
#ggsave("Palleja_Suez_GMM_predict_GMM_vegdist_correlation1.pdf", device = "pdf", width = 12, height = 7)

(g7|g8)/(g9|g10) + plot_layout(guides = "collect")
#ggsave("Palleja_Suez_GMM_predict_GMM_vegdist_correlation2.pdf", device = "pdf", width = 12, height = 7)

# Do FDR correction
cor_results <- as.data.frame(matrix(nrow = 10, ncol = 3))
colnames(cor_results) <- c("Feature", "Spearman_rho", "Spearman_p")
cor_results$Feature <- c("MF0042",
                         "MF0048",
                         "MF0030",
                         "MF0033",
                         "MF0019",
